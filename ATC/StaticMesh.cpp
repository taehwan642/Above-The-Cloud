...
HRESULT StaticMesh::LoadMesh(wstring _filepath, wstring _filename)
{
	...
	{
		oriMesh->CloneMeshFVF(oriMesh->GetOptions(), FVF | D3DFVF_NORMAL, DXUTGetD3D9Device(), &mesh);
		D3DXComputeNormals(mesh, static_cast<ULONG*>(adjacencyBuffer->GetBufferPointer()));
	}
	else
	{
		oriMesh->CloneMeshFVF(oriMesh->GetOptions(), FVF, DXUTGetD3D9Device(), &mesh);
	}
	...
	for (ULONG i = 0; i < subsetCount; ++i)
	{
		...
		hr = D3DXCreateTextureFromFile(DXUTGetD3D9Device(), 
									   finalpath, 
									   &textures[i]);
	}
